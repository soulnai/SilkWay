...
namespace TMPro.EditorUtilities
{
    ...
    public static class TMPro_EditorUtility
    {
        ...
        public static Material[] FindMaterialReferences(Material mat)
        {
            ...
            {
                if (mats[i].HasProperty(ShaderUtilities.ID_MainTex) && mats[i].mainTexture != null && mats[i].mainTexture.GetInstanceID() == mat.mainTexture.GetInstanceID())
                {
                    refs.Add(mats[i]);
                    //Debug.Log(mats[i].name + "  Texture ID:" + mats[i].mainTexture.GetInstanceID());
                }
            }
