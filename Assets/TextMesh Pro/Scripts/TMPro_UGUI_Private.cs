...
{  
    public partial class TextMeshProUGUI
    {
        ...
        protected override void Awake()
        {
            ...
            if (m_settings != null)
            {
                if (m_text == null)
                {
                    ...
                    m_tintAllSprites = m_settings.enableTintAllSprites;
        ...
        protected override void OnDisable()
        {
            ...
            this.canvasRenderer.Clear();  // Might not want to clear since it wipes the Material in addition to the mesh geometry.
        ...
        protected override int SetArraySizes(int[] chars)
        {
            ...
            for (int i = 0; chars[i] != 0; i++)
            {
                ...
                #region LOOKUP GLYPH
                m_currentFontAsset.characterDictionary.TryGetValue(c, out glyph);
                if (glyph == null)
                {
                    // Check Fallback Font Assets
                    if (m_settings != null && m_settings.fallbackFontAssets != null)
                    {
                        // Iterate through each Fallback Font Asset looking for the missing character.
                        for (int y = 0; y < m_settings.fallbackFontAssets.Count; y++)
                        {
                            TMP_FontAsset fontAsset = m_settings.fallbackFontAssets[y];

                            if (fontAsset == null) continue;

                            if (fontAsset.characterDictionary.TryGetValue(c, out glyph))
                            {
                    ...
                    if (glyph == null)
                    {
                        m_currentFontAsset.characterDictionary.TryGetValue(9633, out glyph);
                        if (glyph != null)
                        {
                            if (!m_warningsDisabled) Debug.LogWarning("Character with ASCII value of " + c + " was not found in the Font Asset Glyph Table.", this);
                        ...
                        else
                        {
                            ...
                            TMP_FontAsset fontAsset = TMP_FontAsset.defaultFontAsset;
                            if (fontAsset != null && fontAsset.characterDictionary.TryGetValue(9633, out glyph))
                            {
        ...
        protected override void GenerateTextMesh()
        {
            ...
            for (int i = 0; m_char_buffer[i] != 0; i++)
            {
                ...
                if (!isUsingAltTypeface && (m_style & FontStyles.Bold) == FontStyles.Bold || (m_fontStyle & FontStyles.Bold) == FontStyles.Bold) // Checks for any combination of Bold Style.
                {
                ...
                if (!isUsingAltTypeface && (m_style & FontStyles.Italic) == FontStyles.Italic || (m_fontStyle & FontStyles.Italic) == FontStyles.Italic)
                {
