...
namespace TMPro
{
    ...
    public class TMP_Text : MaskableGraphic
    {
        ...
        public TMP_FontAsset font
        {
            ...
            set { if (m_fontAsset == value) return;  m_fontAsset = value; LoadFontAsset(); m_havePropertiesChanged = true; m_isCalculateSizeRequired = true; SetVerticesDirty(); SetLayoutDirty(); }
        }
        [SerializeField]
        ...
        protected virtual void SaveSpriteVertexInfo(Color32 vertexColor)
        {
            ...
            else
            {
                ...
                {
                    m_textInfo.characterInfo[m_characterCount].vertex_BL.color = m_fontColorGradient.bottomLeft * spriteColor;
                    m_textInfo.characterInfo[m_characterCount].vertex_TL.color = m_fontColorGradient.topLeft * spriteColor;
                    m_textInfo.characterInfo[m_characterCount].vertex_TR.color = m_fontColorGradient.topRight * spriteColor;
                    m_textInfo.characterInfo[m_characterCount].vertex_BR.color = m_fontColorGradient.bottomRight * spriteColor;
                }
            }
        ...
        protected void GetSpecialCharacters(TMP_FontAsset fontAsset)
        {
            ...
            if (!fontAsset.characterDictionary.TryGetValue(95, out m_cached_Underline_GlyphInfo)) //95
            {
                ...
                if (m_settings != null && !m_settings.warningsDisabled)
                    Debug.LogWarning("Underscore character wasn't found in the current Font Asset [" + fontAsset.name + "]. No characters assigned for Underline.", this);
            }
            ...
            if (!fontAsset.characterDictionary.TryGetValue(8230, out m_cached_Ellipsis_GlyphInfo)) //95
            {
                ...
                if (m_settings != null && !m_settings.warningsDisabled)
                    Debug.LogWarning("Ellipsis character wasn't found in the current Font Asset [" + fontAsset.name + "]. No characters assigned for Ellipsis.", this);
            }
        }
        ...
        {
            double x0 = Math.Floor(x * 4095);
            double y0 = Math.Floor(y * 4095);
                ...
                switch (m_xmlAttribute[0].nameHashCode)
                {
                    ...
                    case 1913798: // <action>
                        if (m_isParsingText)
                        {
                            ...
                            Debug.Log("First character index: " + m_characterCount);
                        }
                        return true;
                    case 7443301: // </action>
                        if (m_isParsingText)
                        {
                            Debug.Log("Last character index: " + (m_characterCount - 1));
                        }
                        return true;
                }
            }
